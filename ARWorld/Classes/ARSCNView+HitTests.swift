...
extension ARSCNView {
    ...
    func hitTestWithInfiniteHorizontalPlane(_ point: CGPoint, _ pointOnPlane: float3) -> float3? {
        ...
        // Don not intersect with planes above the camera or if the ray is almost parallel to the plane.
        if ray.direction.y > -0.03 {
            return nil
        }
        
        // Return the intersection of a ray from the camera through the screen position with a horizontal plane at height (Y axis).
        return rayIntersectionWithHorizontalPlane(rayOrigin: ray.origin, direction: ray.direction, planeY: pointOnPlane.y)
    }
    
    func hitTestWithFeatures(_ point: CGPoint, coneOpeningAngleinDegress: Float, minDistance: Float = 0, maxDistance: Float = Float.greatestFiniteMagnitude, maxResults: Int = 1) -> [FeatureHitTestResult] {
        ...
        let maxAngleInDeg = min(coneOpeningAngleinDegress, 360) / 2
        ...
        for i in 0...features.__count {
            ...
            if angleBetweenRayAndFeature > maxAngle {
                // Skip this feature - it is outside of the hit test cone.
                continue
            }
            
            // All tests passed: Addd the hit againts this feature to the results.
