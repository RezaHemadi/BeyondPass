//
//  PortalDecorationCollectionViewCell.swift
//  ARWorld
//
...
class PortalDecorationCollectionViewCell: UICollectionViewCell {
    
    let activityIndicator = UIActivityIndicatorView(style: .whiteLarge)
    
    var decorationModelObject: PFObject?
    ...
    func loadData() {
        /// We are working with the PurchasedPortalModel table
        if let purchasedModel = self.decorationModelObject {
            let decorationModel = purchasedModel["DecorationModel"] as! PFObject
            decorationModel.fetchInBackground { (object: PFObject?, error: Error?) -> Void in
                let imageFile = decorationModel["Image"] as! PFFileObject
                imageFile.getDataInBackground {(data: Data?, error: Error?) -> Void in
                    if error == nil {
                        if let imageData = data {
                            self.image = UIImage(data: imageData)
                            let name = decorationModel["Name"] as! String
                            self.name = name
